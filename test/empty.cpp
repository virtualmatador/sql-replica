...
bool t01() {
  ...
  const auto sql =
      Schema(schema(empty, empty, empty, empty), true, true).replicate_sql();
...
bool t02() {
  auto empty = read_json("[]");
  const auto sql =
      Schema(schema(empty, empty, empty, empty), true, false).replicate_sql();
  return expect_contains(sql,
                         "set @_sql_tables = if(isnull(@old_db), json_array()",
                         __FUNCTION__) &&
         expect_contains(sql, "select @qry as '';", __FUNCTION__) &&
...
bool t03_null_sections_skip_work() {
  ...
  const auto null_sql =
      Schema(schema(null, null, null, null), true, true).replicate_sql();
  ...
  return expect_not_contains(null_sql, "set @_sql_tables =", __FUNCTION__) &&
         ...
         expect_not_contains(null_sql, "set @all_functions =", __FUNCTION__) &&
         expect_not_contains(null_sql, "set @all_procedures =", __FUNCTION__) &&
...
bool t04_empty_sections_reconcile_work() {
  ...
  const auto table_sql =
      Schema(schema(empty, null, null, null), true, true).replicate_sql();
  const auto function_sql =
      Schema(schema(null, empty, null, null), true, true).replicate_sql();
  const auto procedure_sql =
      Schema(schema(null, null, empty, null), true, true).replicate_sql();
  const auto user_sql =
      Schema(schema(null, null, null, empty), true, true).replicate_sql();

  return expect_contains(table_sql, "set @all_tables = '';", __FUNCTION__) &&
         expect_contains(table_sql, "DROP TABLE", __FUNCTION__) &&
         expect_not_contains(table_sql, "set @all_functions = '';",
                             __FUNCTION__) &&
         expect_contains(function_sql, "set @all_functions = '';",
                         __FUNCTION__) &&
         expect_contains(function_sql, "DROP FUNCTION", __FUNCTION__) &&
         expect_not_contains(function_sql, "set @all_tables = '';",
                             __FUNCTION__) &&
         expect_contains(procedure_sql, "set @all_procedures = '';",
                         __FUNCTION__) &&
         expect_contains(procedure_sql, "DROP PROCEDURE", __FUNCTION__) &&
         expect_not_contains(procedure_sql, "set @all_functions = '';",
                             __FUNCTION__) &&
